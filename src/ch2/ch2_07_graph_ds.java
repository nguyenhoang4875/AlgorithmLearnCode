...
class pair1<X, Y> { // utilizing Java "Generics"
    X _first;
    Y _second;

    public pair1(X f, Y s) {
        _first = f;
        _second = s;
    }
...
public class ch2_07_graph_ds {
    ...
    */
        ...
        File f = new File("src/ch2/in_07.txt");
        ...
        V = sc.nextInt();
        Vector<Vector<pair1<Integer, Integer>>> AdjList = new Vector<Vector<pair1<Integer, Integer>>>(V);
        for (int i = 0; i < V; i++) { // for each vertex
            Vector<pair1<Integer, Integer>> Neighbor = new Vector<pair1<Integer, Integer>>();
        ...
        for (int i = 0; i < V; i++) {
            ...
            for (int j = 0; j < total_neighbors; j++) {
                id = sc.nextInt();
                weight = sc.nextInt();
                AdjList.get(i).add(new pair1<Integer, Integer>(id - 1, weight)); // some index adjustment
            }
        }
        ...
        while (it.hasNext()) { // O(k), where k is the number of neighbors
            pair1<Integer, Integer> val = (pair1<Integer, Integer>) it.next();
            System.out.println("Edge 0-" + val.first() + " (weight = " + val.second() + ")");
        }

        E = sc.nextInt();
        PriorityQueue<pair1<Integer, pair1<Integer, Integer>>> EdgeList = new PriorityQueue<pair1<Integer, pair1<Integer, Integer>>>(1,
                new Comparator<pair1<Integer, pair1<Integer, Integer>>>() { // overriding the compare method
                    public int compare(pair1<Integer, pair1<Integer, Integer>> i, pair1<Integer, pair1<Integer, Integer>> j) {
        ...
        for (int i = 0; i < E; i++) {
            a = sc.nextInt();
            b = sc.nextInt();
            pair1<Integer, Integer> ab = new pair1<Integer, Integer>(a, b);
            weight = sc.nextInt();
            EdgeList.offer(new pair1<Integer, pair1<Integer, Integer>>
                    (-weight, ab)); // trick to reverse sort order */
        }

        // edges sorted by weight (smallest->largest)
        for (int i = 0; i < E; i++) {
            pair1<Integer, pair1<Integer, Integer>> edge = EdgeList.poll();
